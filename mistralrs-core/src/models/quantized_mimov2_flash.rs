...
#[derive(Debug, Clone)]
#[allow(dead_code)]
pub struct MiMoV2FlashMoEConfig {
...
pub(crate) struct PropsGGUF {
    ...
    pub moe_cfg: MiMoV2FlashMoEConfig,
}

fn verify_mimov2flash_arch(
    ...
    if actual_arch != "mimov2flash" {
        candle_core::bail!("Expected `mimov2flash` architecture, got `{actual_arch}`.");
    }
    Ok(actual_arch)
}
...
impl TryFrom<ContentMetadata<'_>> for PropsGGUF {
    ...
    fn try_from(c: ContentMetadata) -> std::result::Result<Self, Self::Error> {
        let _ = verify_mimov2flash_arch(c.metadata)?;
        ...
        let moe_cfg = MiMoV2FlashMoEConfig {
...
impl ModelConfig::FromGGUF for ModelWeights {
    fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let actual_arch = verify_mimov2flash_arch(meta)?;
        ...
        for layer_idx in NiceProgressBar::<_, 'b'>(
            ...
            let paged_attn = match &attention_mechanism {
                ...
                AttentionImplementation::PagedAttention => {
                    Some(PagedAttention::new(head_dim, device, None)?) as Option<PagedAttention>
                }
            };
